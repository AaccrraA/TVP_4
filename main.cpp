...
private:
    ...
    vector<int> FSS; // Вектор конечных состояний
	...
	void loadFromFile() {
		...
		string rStr;
		if (ifs.fail()) {
			cerr << "РћС€РёР±РєР° РѕС‚РєСЂС‹С‚РёСЏ С„Р°Р№Р»Р°.";
			exit(-1);
		}
		do {
			getline(ifs, field, '\n');
...
public:
	...
	void buildFSM() {
		...
		for (int i = 0; i < X.length(); i++) {
			// -1 - РѕС‚СЃСѓС‚СЃС‚РІСѓРµС‚ РїРµСЂРµС…РѕРґ РІ РґСЂСѓРіРѕРµ СЃРѕСЃС‚РѕСЏРЅРёРµ
			A.push_back(-1);
		}
...
//	setlocale(LC_ALL, "rus");
    ...
    C.printFSM();
    cout << "Упрощенный конечный автомат:" << endl;
    C.optimizeFSM();
    C.printFSM();
